...
# master    : DandyRunner
# project   : https://github.com/DandyRunner/man-made-arch
# license   : LGPL-3.0 (http://opensource.org/licenses/lgpl-3.0.html)
#
# referance : https://wiki.archlinux.org/index.php/Installation_guide


apptitle="Man Made Arch - Linux Install Script - (GPLv3)"
baseurl=https://raw.githubusercontent.com/dandyrunner/man-made-arch/master
cpl=0
skipfont="0"
fspkgs=""
...
installbase(){
	...
	if [[ ${cpl} == 1 ]]; then
		sed -i '/^\s*$/d' /tmp/mma-custom-package-list
		sed -i '/^#/ d' /tmp/mma-custom-package-list
		options=()
		while read pkg; do
			options+=("${pkg}" "" on)
		done < /tmp/mma-custom-package-list
...
while (( "$#" )); do
	case ${1} in
		--help)
			...
			echo "-t | --test ghusername ghbranch   Test mma script and language"
			exit 0
		;;
		-cpl | --custom-package-list)
			curl -L ${2} > /tmp/mma-custom-package-list
			if [[ "$?" != "0" ]]; then
